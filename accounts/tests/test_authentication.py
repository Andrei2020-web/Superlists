...
class AuthenticateTest(TestCase):
    ...
    def test_returns_None_if_no_such_token(self):
        ...
        result = PasswordlessAuthenticationBackend().authenticate(request=None,
                                                                  uid='no-such-token'
                                                                  )
    ...
    def test_returns_new_user_with_correct_email_if_token_exists(self):
        ...
        user = PasswordlessAuthenticationBackend().authenticate(request=None,
                                                                uid=token.uid)
        new_user = User.objects.get(email=email)
        self.assertEqual(user, new_user)

    def test_returns_existing_user_with_correct_email_if_token_exist(self):
        '''тест: возвращантся существующий пользователь с правильной
        электронной почтой, если маркер существует'''
        email = 'edith@example.com'
